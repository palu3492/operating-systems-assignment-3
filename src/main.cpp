...

        {
            ...
            for (i = 0; i < processes.size(); i++) {
                ...
                if (processes[i]->getState() != Process::State::Terminated) {
                    ...
                    //@@See if time
                    if ((currentTime() - start) >= processes[i]->getStartTime() &&
    ...
    while (!shared_data->all_terminated) {
        ...
        if (!shared_data->all_terminated) {
            ...
            if (algorithm == ScheduleAlgorithm::FCFS) {
                ...
                while (currentTime() - start_cpu_time < cpu_burst_time) {
                    ...
                    if (shared_data->ready_queue.front() != NULL &&
                        runningProcess->getPriority() > shared_data->ready_queue.front()->getPriority()) {
                        break;
                    }
            ...
            {//* CRITICAL SECTION GET FROM READ QUEUE
                ...
                runningProcess->updateProcess(
                        currentTime()); // moves to next burst if more bursts and finished burst above
                //Terminate
